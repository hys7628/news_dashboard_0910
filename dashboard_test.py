...
# 3. 디자인 스타일링 (업로드한 레퍼런스 스타일의 파스텔 오로라 & 글래스모피즘 적용)
st.markdown("""
<style>
    ...
    * {
    ...
    /* 2. 레퍼런스 중앙의 거대한 화이트 반투명 컨테이너 패널 */
    .main .block-container {
        ...
        box-shadow: 0 30px 70px rgba(80, 100, 140, 0.12),
    ...
    @keyframes floating {
        ...
        50% { transform: translateY(-16px) scale(1.02); box-shadow: 0 25px 45px rgba(50, 80, 150, 0.28); }
    ...
    /* 기본 보조 버튼 스타일 (레퍼런스의 둥근 캡슐형 알약 버튼 스타일) */
    ...
    if not categorized_data:
        ...
        for idx, category in enumerate(categories):
            ...
            with col:
                ...
                icon_html = f'<img src="{icon_data_uri}" class="card-left-icon-img" alt="{norm_cat}">' if icon_data_uri else '<div style="width:36px; height:36px; margin-bottom:10px;"></div>'
                ...
                right_img_html = f'<img src="{right_img_uri}" class="card-right-img" alt="{norm_cat}">' if right_img_uri else '<div style="width:85px; height:85px;"></div>'

                # 카드 렌더링 (좌측 이모티콘 PNG + 우측 일러스트 PNG)
                st.markdown(f"""
        ...
        if st.session_state.selected_category:
            ...
            if selected_filename:
                ...
                try:
                    ...
                    _, viewer_col, _ = st.columns([0.05, 0.9, 0.05])
                    with viewer_col:
